...
import { confirmExtrude, startExtrude } from "@/tools/extrudeSession";
 ...
 * The drag itself is tracked by the ExtrudeController, exactly as with E, so snapping, the readout
 * and typed distances all work the same.
 */
export function FaceArrow() {
	...
	const onPointerDown = (e: ThreeEvent<PointerEvent>) => {
		...
		pointer.capture(e);
		startExtrude();
		if (!useAppStore.getState().extrude) {
			pointer.release(e);
			return;
		}
		setDragging(true);
	...
	const color = hovered || dragging ? HOVER_COLOR : COLOR;
	return (
